...
public class TC03_ClassG_ValidationCheck_SubmitCase extends BaseTest
{
	...
	{	
		...
		ut.NEXT_2();         //User selects Account number and click on NEXT
		
		
		s_assert.assertEquals(AccountDetails_heading, ut.Page_heading());
		ut.MandatoryFieldCheck();                //Mandatroy check
		...
		cg.Select_LegalNotice_Issued(Sh_ClassG,1,10);
		cg.Enter_EffectiveDate(Sh_ClassG,1,11);
		...
		String AddScreenClassG=ut.captureScrenshot(Screenshotname);	
	...
	{	
		...
		int j=2; //If 1 selects "This time only" option if 2 Selects "Indefinitely (until you advise us otherwise)"
			...
			if(j==1)
			{
				...
				ut.AddressdelegationTextbox(Sh_ClassG,1,15,3);                 // Parameter are Sheetname, excel filename & path, Row, Colunm, Dropdown index value (Actual address)  
